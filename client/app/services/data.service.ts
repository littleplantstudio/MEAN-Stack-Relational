...
export class DataService {
  ...
  getCat(cat): Observable<any> {
    return this.http.get(`/api/cat/${cat.id}`, this.options);
  }

  editCat(cat): Observable<any> {
    return this.http.put(`/api/cat/${cat.id}`, JSON.stringify(cat), this.options);
  }

  deleteCat(cat): Observable<any> {
    return this.http.delete(`/api/cat/${cat.id}`, this.options);
  }
